...
export function ServicesSection({ section }: ServicesSectionProps) {
  ...
  if (fields.services?.value) {
    try {
      services = JSON.parse(fields.services.value);
    } catch {
      services = defaultServices;
    }
  }

  const title = fields.title?.value || 'Solusi Audio Terintegrasi';
  const description = fields.description?.value || 'Kami menyediakan lebih dari sekadar peralatan. PLS memberikan ketenangan pikiran melalui layanan teknis yang komprehensif.';
